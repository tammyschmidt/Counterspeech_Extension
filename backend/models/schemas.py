...
class GenerateRequest(BaseModel):
    """Request model for generating counterspeech suggestions"""
    ...
    use_placeholders: bool = Field(default=False, description="Whether to include explicit placeholders the user can fill with personal details")
...
class GenerateResponse(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "suggestions": [
                    {"text": "First counterspeech suggestion"},
                    {"text": "Second counterspeech suggestion"},
                    {"text": "Third counterspeech suggestion"}
                ]
            }
        }
